...
func WsPartialDepthServe100Ms(symbol string, levels string, handler WsPartialDepthHandler, errHandler ErrHandler, isUS bool) (doneC, stopC chan struct{}, err error) {
	url := getWsEndpoint()
	if isUS {
	    url = baseWsUSURL
	}
	endpoint := fmt.Sprintf("%s/%s@depth%s@100ms", url, strings.ToLower(symbol), levels)
	return wsPartialDepthServe(endpoint, symbol, handler, errHandler)
}
...
func WsDepthServe100Ms(symbol string, handler WsDepthHandler, errHandler ErrHandler, isUS bool) (doneC, stopC chan struct{}, err error) {
	url := getWsEndpoint()
	if isUS {
	    url = baseWsUSURL
	}
	endpoint := fmt.Sprintf("%s/%s@depth@100ms", url, strings.ToLower(symbol))
	return wsDepthServe(endpoint, handler, errHandler)
}
...
func WsBookTickerServe(symbol string, handler WsBookTickerHandler, errHandler ErrHandler, isUS bool) (doneC, stopC chan struct{}, err error) {
	url := getWsEndpoint()
	if isUS {
	    url = baseWsUSURL
	}
	endpoint := fmt.Sprintf("%s/%s@bookTicker", url, strings.ToLower(symbol))
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
